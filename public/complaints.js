...
(function () {
  ...
  async function renderDashboard() {
    ...
    c.appendChild(panel('По агентам',
      'Реакция — сколько прошло от подачи до «Принял в работу» (медиана), и только по претензиям, '
      + 'которые подал сам клиент: свою собственную агент «принимает» через секунду. '
      + '«От клиента» — сколько таких претензий было.',
  ...
  function agentTable(byAgent) {
    ...
    const body = byAgent.map((a) => el('tr', {
      ...
      el('td', { class: 'n' }, String(a.ot_klienta || 0)),
    ...
    return el('table', { class: 't cmp-agent-t' }, [
      el('thead', {}, el('tr', {}, [el('th', {}, 'Агент'), el('th', {}, 'Претензий'),
        el('th', { title: 'Сколько претензий подал сам клиент, а не агент за него' }, 'От клиента'),
        el('th', { title: 'Медиана времени от подачи клиентом до «Принял в работу». Претензии, заведённые самим агентом, не учитываются' }, 'Реакция'),
