...
(function () {
  ...
  const SHEET_ABOUT = {
    ...
    vinegar: 'Покупной концентрат в импортной бутылке. Зелени и общего производства здесь нет, '
      + 'поэтому упаковка, производственные затраты и ФОТ вводятся прямо на товаре. '
      + 'Ретро сети, НДС и налог на прибыль считаются так же, как на остальных листах.',
  };
  // Товарные листы устроены одинаково, поэтому у них общий загрузчик и общий вид.
  const SKU_SHEETS = ['retail', 'horeca250', 'horeca500', 'salads', 'bunches', 'culinary', 'cutveg', 'vinegar'];
  ...
  function sbTable() {
    const HEAD = [
      ...
      ['Вклад с ед.', 'Цена минус то, что растёт вместе с объёмом: сырьё, упаковка, ретро, НДС.\n\nЭто деньги, которые одна единица приносит на покрытие аренды и зарплат. Именно вклад режет скидка.'],
      ['Вклад всего', 'Вклад с единицы × объём.\n\nСравниваем эти два числа: если справа не меньше, чем слева, — скидка себя окупила.'],
      ['', null],
    ];
    const head = el('tr', {}, HEAD.map((h, i) => el('th', {
      class: i && i < 7 ? 'tnum' : '', 'data-hint': h[1] || null,
    }, h[0])));
  ...
  // Что за строка и от чего она зависит. Главное, что нужно понять: одни
  // расходы от цены не зависят вовсе, другие считаются процентом от неё —
  // отсюда и разное поведение при скидке.
  const SB_PART_HINT = {
    raw: 'Сырьё: рецептура или граммаж × цена за кг.\n\nОт цены продажи не зависит — при скидке остаётся тем же.',
    pack: 'Комплект упаковки с листа «Упаковка».\n\nОт цены продажи не зависит.',
    labor: 'Фонд оплаты труда, поделённый на общий выпуск завода.\n\nМеняется только рычагом «Выпуск завода» наверху, от объёма сделки не зависит.',
    production: 'Расходы завода за месяц, поделённые на выпуск.\n\nМеняется только рычагом «Выпуск завода» наверху.',
    defect: 'Надбавка на брак: процент, заложенный на листе товара.',
    retro: 'Ретро-бонус сети — процент от цены.\n\nЦена ниже — и ретро меньше, поэтому скидка бьёт по вам не на всю сумму.',
    vat: 'НДС — процент от цены.\n\nЦена ниже — и НДС меньше, поэтому скидка бьёт по вам не на всю сумму.',
    tax: 'Налог на прибыль — процент от прибыли.',
    margin: 'Что остаётся компании.\n\nИменно отсюда платится скидка: расходы от неё не уменьшаются.',
  };
  ...
  function sbParts(x) {
    ...
    const rows = (x.now.parts || []).map((p) => {
      ...
      return el('tr', { class: p.key === 'margin' ? 'calc-sb-margin' : '', 'data-hint': SB_PART_HINT[p.key] || null }, [
        el('td', {}, p.label),
    ...
    const head = el('tr', {}, [
      ...
      class: i ? 'tnum' : '',
      'data-hint': i === 1 || i === 3 ? 'Доля от цены. Все строки вместе с маржой дают ровно 100%.' : null,
    }, t)));
    ...
    // Главное недоразумение этой таблицы: суммы не изменились, а проценты
    // изменились. Объясняем прямо здесь, а не подсказкой при наведении.
    const dropped = x.price !== null && x.price_new !== null && x.price_new < x.price;
    const why = el('div', { class: 'calc-sb-why' }, dropped
      ? 'Проценты — это доля от цены. Цена упала, а расходы остались те же, поэтому их доли выросли: '
        + 'та же сумма занимает больше места в меньшей цене. Сжалась только маржа — '
        + 'скидку вы платите из неё.'
      : 'Проценты — это доля от цены. Все строки вместе с маржой дают ровно 100%.');
    return el('div', {}, [
      why,
