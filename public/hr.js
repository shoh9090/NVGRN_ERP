...
(function () {
  ...
  async function renderDashboard() {
    ...
    box.appendChild(el('div', { class: 'hr-kpis hr-kpis-4' }, [
      ...
      // Клик по плитке всегда показывает то, что на ней написано: кому сколько
      // осталось выплатить. Переплата — сноска рядом, у неё своя ссылка: раньше
      // она подменяла собой разрез по плитке, и вместо остатков открывался
      // список из одного человека.
      kpi('= К выплате', moneyShort(t.to_pay), 'green',
        () => openBreakdown('К выплате (остаток)', d.emps || [], (r) => Math.max(0, r.to_pay), dashState.period),
        'клик — кому сколько'),
    ]));
    const hasOverpay = t.overpay >= 1 && (d.overpaid || []).length;
    box.appendChild(el('div', { class: 'hr-sub', style: 'margin:2px 0 6px' }, [
      'Начислено − Удержания − Выплачено (с авансами) = К выплате.',
      hasOverpay ? ' ' : null,
      hasOverpay ? el('a', { href: 'javascript:void(0)', onclick: () => openOverpaid(d.overpaid) },
        'Переплата ' + moneyShort(t.overpay)) : null,
      hasOverpay ? ' (кому-то выдали больше — на общий остаток не влияет).' : null,
      '  ·  Сотрудников: ' + t.count,
    ]));
  ...
  function openEmp(e) {
    ...
    const body = el('div', { class: 'hrf' }, [
      ...
      e.id ? el('div', { class: 'hr-sub', style: 'margin:0 0 6px' }, e.trello_username
        ? 'Trello: @' + e.trello_username + ' — сопоставлен в плитке «Джарвис».'
        : 'Trello: не сопоставлен. Сопоставляется в плитке «Джарвис».') : null,
    ]);
  ...
  //   10      — отработал смену
  //   13      — смена плюс переработка: при норме 10 это 10 +3
  ...
  async function renderMassOps() {
    ...
    c.appendChild(el('div', { class: 'hr-head' }, [
      el('div', {}, [el('div', { class: 'hr-h2' }, 'Массовые операции — ' + monthLabel(massState.period)),
        ...
        // Кадровые действия (перевод, увольнение, архив) живут в «Сотрудниках»:
        // это не суммы за месяц, а изменение самой карточки. Подсказываем, где
        // их искать, — иначе их ищут здесь по названию вкладки.
        el('div', { class: 'hr-sub', style: 'margin-top:4px' }, [
          'Здесь только суммы. Групповой перевод в другой отдел или на другой график, увольнение и архив — во вкладке ',
          el('a', { href: 'javascript:void(0)', onclick: () => { TAB = 'employees'; render(); } }, '«Сотрудники»'),
          ': отметьте людей галочками, внизу появится полоса с действиями.',
        ])]),
    ]));
    const mInp = HubDateRange.create({
