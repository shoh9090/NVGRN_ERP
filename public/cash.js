...
(function () {
    ...
    if (profit === null) {
      ...
      phrase = 'За ' + monthLabelRu(d.period) + ' отгрузили на ' + mlrd(d.revenue.total)
        + (d.revenue.receivable > 0
          ? (', денег из них пришло ' + mlrd(d.revenue.cash_in)
            + ' — остальное в отсрочке')
          : '')
  ...
  function pnlCalc(box, d) {
    ...
    if (d.revenue.source === 'shipped') {
      ...
      row('   отгружено, но не оплачено', money(d.revenue.receivable), {
        cls: 'cash-pnl-sub' + (d.revenue.receivable < 0 ? ' cash-pnl-bad' : ''),
        hint: d.revenue.receivable >= 0
          ? 'Деньги в отсрочке у клиентов. Если растёт месяц к месяцу — деньги зависают.'
          : 'Денег пришло больше, чем отгружено: закрыты долги прошлых месяцев.',
      });
    }
  ...
  function pnlSources(box, d) {
    ...
    if (d.revenue.source === 'shipped') {
      ...
      line('   отгружено, но не оплачено', 'Разница. При отсрочке до 30 дней это нормально; тревожно, если растёт месяц к месяцу', d.revenue.receivable);
    }
    ...
    line('− возвраты от поставщиков', 'Приход, пришедший на РАСХОДНУЮ статью. Уменьшает расход, а не создаёт выручку', rc.other_inflows);
