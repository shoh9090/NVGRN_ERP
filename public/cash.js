...
(function () {
  ...
  function loadRatios(d) {
    ...
    const items = [
      ...
      // Потери по статьям списания — то, что выбросили со склада уже после
      // приёмки. От отходов отличается тем, что это не обрезь при разборе,
      // а испорченное, усохшее и недостача.
      one('Потери со склада', r.writeoff_pct,
        d.writeoff && d.writeoff.has_data
          ? (Object.entries(d.writeoff.by_reason || {}).sort((a, b) => b[1] - a[1])
            .map(([k, v]) => k + ' ' + money(v)).join(' · ')
            + (d.writeoff.supplier ? '. Брак поставщика ' + money(d.writeoff.supplier) + ' — в потери не входит' : ''))
          : 'за месяц списаний не было', CH.loss),
    ...
    if (profit === null) {
      ...
      phrase = 'За ' + monthLabelRu(d.period) + ' отгрузили на ' + mlrd(d.revenue.total)
        + (d.revenue.receivable > 0
          ? (', денег за месяц пришло ' + mlrd(d.revenue.cash_in)
  ...
  function pnlCalc(box, d) {
    ...
    row('Выручка (реализация)', money(d.revenue.total), {
      cls: 'cash-pnl-head',
      hint: d.revenue.source === 'shipped'
        ? 'Отгружено за месяц по SalesDoctor, за вычетом возвратов. Именно отгрузка, а не приход денег: при отсрочке до 30 дней деньги приходят в следующем месяце.'
        : 'Реализация из SalesDoctor не подтянута, поэтому взят приход денег. Это занижает выручку на сумму отсрочек — нажмите «обновить» внизу.',
    });
    if (d.revenue.source === 'shipped') {
      row('   в том числе получено деньгами', money(d.revenue.cash_in), {
        cls: 'cash-pnl-sub', hint: 'Приход по доходным статьям Кассы за этот же месяц.',
      });
      row('   разница: отгрузка минус поступления', money(d.revenue.receivable), {
        cls: 'cash-pnl-sub' + (d.revenue.receivable < 0 ? ' cash-pnl-bad' : ''),
        hint: 'Это НЕ долг клиентов за месяц: в поступлениях августа могут быть оплаты июльских отгрузок '
          + 'и авансы за сентябрь. Показывает, насколько деньги отстают от отгрузок. '
          + (d.revenue.receivable >= 0
            ? 'Растёт месяц к месяцу — деньги зависают у клиентов.'
            : 'Минус значит, что денег пришло больше, чем отгрузили: закрывали старые долги.'),
      });
    }
    row('Поступления по статьям Кассы', money(d.revenue.cash_in), {
      cls: 'cash-pnl-sub', hint: 'Справочно, для сверки с Кэш-флоу. В прибыль идёт реализация.',
    });
    d.revenue.sales_items.forEach((x) => row('   ' + x.code + ' · ' + x.name, money(x.inc), { cls: 'cash-pnl-sub' }));
    if (d.revenue.other) {
      row('   прочие доходы', money(d.revenue.other), {
        cls: 'cash-pnl-sub',
        hint: 'Другие статьи группы «Доходы и поступления», кроме 200.',
      });
    }
    ...
    if (rc) {
      ...
      recRow('− возвраты и прочие поступления', money(rc.other_inflows),
    ...
    if (ex.other_inflows && ex.other_inflows.total) {
      exRow('Возвраты и прочие поступления', money(ex.other_inflows.total),
        ...
        + '. В выручку не входит и расход месяца не уменьшает — за какой месяц вернули деньги, видно только по документу.');
    }
  ...
  // ---------------------------------------------------------------------------
  // Вкладка «Откуда цифры»
  // ---------------------------------------------------------------------------
  // Три столбца: статья, откуда она берётся, значение. Никаких сворачиваний —
  // весь путь от «пришло денег» до прибыли одной лентой, чтобы можно было
  // ткнуть пальцем в строку, где цифра стала неожиданной.
  //
  // Это временный инструмент настройки: пока данные не выверены, важнее видеть
  // источник каждой цифры, чем красиво её показать.
  function pnlSources(box, d) {
    const rows = [];
    const line = (name, where, value, cls) => rows.push(
      el('tr', { class: 'cash-src-r' + (cls ? ' ' + cls : '') }, [
        el('td', { class: 'cash-src-name' }, name),
        el('td', { class: 'cash-src-where' }, where),
        el('td', { class: 'cash-src-val' }, value === null || value === undefined ? '—' : money(value)),
      ]));
    const head = (name) => rows.push(el('tr', { class: 'cash-src-head' },
      el('td', { colspan: '3' }, name)));

    const rc = d.reconcile || {};

    // --- 1. Откуда взялась выручка ---
    head('1. Выручка — это ОТГРУЗКА, а не приход денег');
    line('Выручка (реализация)', d.revenue.source === 'shipped'
      ? 'SalesDoctor → заказы за месяц в статусах «отправлен», «доставлен», «закрыт», за вычетом возвратов. Обновляется кнопкой внизу'
      : 'Реализация НЕ подтянута — взят приход денег. При отсрочке это занижает выручку и даёт мнимый убыток',
    d.revenue.total, d.revenue.source === 'shipped' ? 'cash-src-total' : 'cash-src-warn');
    if (d.revenue.source === 'shipped') {
      line('   из них деньгами получено', 'Приход по доходным статьям Кассы за этот же месяц', d.revenue.cash_in);
      line('   разница: отгрузка минус поступления',
        'Отгрузка месяца минус деньги, пришедшие в этом же месяце. Долгом клиентов это назвать нельзя: '
        + 'в поступлениях есть оплаты прошлых месяцев и авансы. Для настоящей дебиторки нужна сверка с SalesDoctor по клиентам',
        d.revenue.receivable);
    }

    head('1a. Поступления денег — справочно, для сверки с Кэш-флоу');
    line('Всего пришло денег за месяц', 'Касса → все приходы за месяц, без переводов между счетами. Столько же на вкладке «Кэш-флоу»', rc.all_in);
    line('− переводы между своими счетами', 'Статьи 100, 110, 111 (помечены как «перевод»). Деньги переложены со счёта на счёт', rc.transfers_in);
    line('− конверсия валюты', 'Статья 102. Купили или продали свою же валюту — ни доход, ни расход', rc.conversion_in);
    line('− кредиты, займы, взносы учредителей', 'Статьи 201, 202, 203 и группа «6. Финансы». Это привлечённые деньги, а не заработок', rc.finance_in);
    line('− возвраты от поставщиков', 'Приход, пришедший на РАСХОДНУЮ статью. Выручкой он не считается, '
      + 'но и расход месяца пока НЕ уменьшает: неизвестно, за какой месяц вернули. Разбирается вручную', rc.other_inflows);
    line('− приходы без статьи', 'Операции, которым ещё не назначена статья ДДС. В отчёт не попадают', rc.unclassified_in);
    line('− возвраты покупателям', 'Расход, ушедший с ДОХОДНОЙ статьи. Уменьшает выручку', rc.refunds);
    line('Выручка от продаж (статья 200)',
      'ЭТУ цифру показывает строка «Выручка от продаж» в Кэш-флоу. В прибыль она НЕ идёт — там реализация',
      d.revenue.sales);
    if (d.revenue.other) {
      line('+ прочие доходы', 'Другие статьи группы «Доходы и поступления», кроме 200: '
        + d.revenue.other_items.map((x) => x.code + ' ' + x.name).join(', ')
        + '. В строку выручки Кэш-флоу они НЕ входят — вот отсюда разница', d.revenue.other, 'cash-src-warn');
    }
    line('Итого поступлений', 'Выручка от продаж плюс прочие доходы минус возвраты покупателям',
      d.revenue.cash_in);
    ...
    head('4. Операционные расходы — по дате ОПЛАТЫ');
    d.opex.groups.slice().sort((a, b) => b.amount - a.amount).forEach((g) => {
      line(g.group_name, 'Группа классификатора ДДС, статей внутри: ' + g.items.length, g.amount, 'cash-src-grp');
      g.items.slice().sort((a, b) => b.exp - a.exp).forEach((i) => {
    ...
    if (d.operating_profit !== null && d.operating_profit < 0) {
      ...
      box.appendChild(el('div', { class: 'cash-src-why' }, [
        el('b', {}, 'Почему минус. '),
        'За месяц денег пришло на ' + mlrd(d.revenue.total)
        ...
        + (d.revenue.source === 'cash'
          ? ' ВНИМАНИЕ: реализация из SalesDoctor не подтянута, выручка взята по поступлению денег.'
            + ' При отсрочке до 30 дней это занижает выручку на целый месяц отгрузок — нажмите «обновить» внизу.'
          : ''),
      ]));
    }

    box.appendChild(el('table', { class: 'cash-src-t' }, [
      el('thead', {}, el('tr', {}, [
        el('th', {}, 'Статья'),
        el('th', {}, 'Откуда берётся'),
        el('th', {}, 'Значение'),
      ])),
      el('tbody', {}, rows),
    ]));
  ...
  function unitsBlock(d) {
    return el('div', { class: 'cash-pnl-units' }, [
      el('div', {}, [
        ...
        el('span', {}, d.revenue.shipped
          ? (money(d.revenue.shipped) + ' сум' + (d.units ? ' · ' + money(d.units) + ' шт' : ''))
          : 'не подтянуто'),
  ...
  async function openCashOpeningForm(walletId) {
    ...
    const body = el('div', { class: 'cashf' }, [
      ...
      el('div', { class: 'cash-sub', style: 'margin:-4px 0 8px;color:#b25b00' }, 'Остаток у кассы ОДИН — это точка старта учёта (самый первый месяц). Сохранение заменит прежний, а остатки следующих месяцев считаются сами из движений.'),
  ...
  async function openMatchTransfers() {
    ...
    const boxes = pairs.map((p) => {
      const cb = el('input', { type: 'checkbox' }); // по умолчанию НЕ отмечено — каждую пару подтверждаем осознанно
