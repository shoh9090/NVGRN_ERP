...
(function () {
  ...
  function humanSummary(d) {
    ...
    const cards = el('div', { class: 'cash-sum-cards' }, [
      card('Продали на', d.revenue.sales, 'статья 200 — как в Кэш-флоу'),
  ...
  function pnlCalc(box, d) {
    ...
    row('Выручка от продаж', money(d.revenue.sales), {
      cls: 'cash-pnl-head',
      hint: 'Статья 200. Ровно та же цифра, что в строке «Выручка от продаж» на вкладке «Кэш-флоу (ДДС)».',
    });
    d.revenue.sales_items.forEach((x) => row('   ' + x.code + ' · ' + x.name, money(x.inc), { cls: 'cash-pnl-sub' }));
    if (d.revenue.other) {
      row('Прочие доходы', money(d.revenue.other), {
        cls: 'cash-pnl-sub',
        hint: 'Другие статьи группы «Доходы и поступления», кроме 200. В строку выручки Кэш-флоу они не входят.',
      });
      d.revenue.other_items.forEach((x) => row('   ' + x.code + ' · ' + x.name, money(x.inc), { cls: 'cash-pnl-sub' }));
    }
    if (d.revenue.refunds) {
      row('Возвраты покупателям', '−' + money(d.revenue.refunds), {
        cls: 'cash-pnl-sub', hint: 'Расход, ушедший с доходной статьи.',
      });
    }
    row('Итого доходов', pnlMoney(d.revenue.total), { cls: 'cash-pnl-total' });
  ...
  function pnlSources(box, d) {
    ...
    head('1. Выручка — из прихода денег в Кассе');
    ...
    line('Выручка от продаж (статья 200)',
      'ЭТУ цифру показывает строка «Выручка от продаж» в Кэш-флоу. Если здесь другое число — расходятся отчёты, а не месяцы',
      d.revenue.sales, 'cash-src-total');
    if (d.revenue.other) {
    ...
    line('Итого доходов', 'Выручка от продаж плюс прочие доходы минус возвраты покупателям. Эта цифра и идёт в прибыль',
      d.revenue.total, 'cash-src-total');
    ...
    if (d.operating_profit !== null && d.operating_profit < 0) {
      ...
      box.appendChild(el('div', { class: 'cash-src-why' }, [
        ...
        + (d.cogs_source === 'plan'
          ...
          : '')
        + ' Частая причина минуса в незакрытом месяце: расходы уже оплачены, а деньги за отгрузки ещё не пришли —'
        + ' выручка здесь считается по поступлению денег, а не по отгрузке.',
      ]));
    }
