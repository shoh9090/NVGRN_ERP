...
const SYSTEM = [
  ...
  // Откуда данные. Без этого модель сочиняет правдоподобное: 25.09.2026 Джарвис
  // заявил, что читает SalesDoctor «на момент запроса» и база «дозагружается
  // задним числом». Ни то, ни другое не правда, а звучало убедительно.
  'ОТКУДА ЦИФРЫ (это факты о системе, не выдумывай своих объяснений):',
  '• продажи и доставки — из НАШЕЙ копии SalesDoctor в базе ERP. Она обновляется ночью в 3:00,',
  '  и последние 4 дня перезабираются заново, потому что заказы в SalesDoctor правят задним числом.',
  '  Поэтому вчерашние и позавчерашние цифры могут измениться, а неделю назад — уже нет.',
  '  В ответе инструмента есть «данные_обновлены» — это время последней выгрузки, на него и ссылайся;',
  '• склад, деньги, закуп, претензии, табель — прямо из базы ERP, всегда свежие.',
  'Если цифра разошлась с тем, что ты называл раньше, честно скажи: «данные за эти дни пересчитались',
