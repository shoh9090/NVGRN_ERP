...
async function ensureCalculationSchema(pool) {
  ...
  for (const [col, type] of [
    ...
    ['recipe_id', 'INTEGER'],
    // Связь с товаром SalesDoctor. Раньше держались только на штрихкоде: нет
    // штрихкода — цена и объёмы из SD не подтянутся, а два одинаковых штрихкода
    // молча подставят чужие цифры. sd_product_id — идентификатор товара из
    // самого СД, вписывается руками: не зависит от того, насколько полон наш
