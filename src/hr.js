...
// больше; начальник производства — ещё и прогноз ФОТ, у него лимит.
// Одной вкладкой «Табель» их не различить, поэтому есть отдельная галочка
// «Суммы в табеле». А тому, у кого и так открыта Зарплата, Выплаты или
// Дашборд, деньги видно без неё — ставить её отдельно было бы странно.
async function canSeeMoney(req) {
  try {
    const ta = require('./tab-access');
    return ta.tabAllowed(await ta.allowedTabs(db.pool, req.user, '/hr'),
      ['ts_money', 'salary', 'payouts', 'dashboard']);
  } catch (e) { return true; }
}
