      ...
      try {
        ...
        if (rules.workspace_id) {
          ...
          for (const b of boards) {
            for (const l of await trello.lists(b.id)) if (R.isDoneList(l.name, rules.done_lists)) done.add(l.name.trim());
          }
...
const LOG_KINDS = ['ai', 'voice', 'remind_silent', 'sales_digest', 'morning', 'remind_no_due', 'violation_no_due', 'due_set', 'due_moved', 'remind_mention', 'violation_mention', 'remind_overdue', 'violation_overdue', 'remind_stale', 'reply'];
