...
async function ensureTables() {
  ...
  // ===== Претензии (Бандл «Претензии», Этап 1: фундамент БД) =====
  // Справочник: типы жалоб, степени, решения, категории, звенья.
  // Один источник и для кнопок бота, и для выпадашек веба. Правится без перезалива кода.
  await db.pool.query(`CREATE TABLE IF NOT EXISTS tgbot.complaint_dicts (
    ...
    link_code  TEXT,                    -- для kind='type': подозреваемое звено (field|production|packing|fulfillment)
  ...
  await db.pool.query(`CREATE TABLE IF NOT EXISTS tgbot.complaints (
    id               BIGSERIAL PRIMARY KEY,
    created_at       TIMESTAMPTZ NOT NULL DEFAULT now(),   -- Дата обращения
    source           TEXT NOT NULL DEFAULT 'client_bot',   -- client_bot | web | import
    -- привязка к точке и агенту
    sd_id            TEXT,
    point_name       TEXT,
    firm_name        TEXT,
    agent_sd_id      TEXT,
    ...
    reporter_phone   TEXT,
    -- привязка к заказу / отгрузке
    order_code       TEXT,
    ship_date        DATE,
    volume_text      TEXT,
    -- суть (заполняет клиент в боте)
    product_sd_id    TEXT,
    product_name     TEXT,
    product_category TEXT,                                 -- premium | eco | retail
    complaint_type   TEXT,                                 -- code из complaint_dicts (kind='type')
    link_code        TEXT,                                 -- звено (денормализовано по типу, для статистики)
    client_comment   TEXT,
    -- условия хранения (опционально)
    storage_place    TEXT,
    storage_temp     NUMERIC,
    open_hours       NUMERIC,
    -- реакция агента (в боте)
    agent_reacted_at TIMESTAMPTZ,
    agent_resolution TEXT,                                 -- code (kind='resolution')
    agent_comment    TEXT,
    agent_tg_id      BIGINT,
    -- обработка в вебе (вы / РОП)
    severity         TEXT,                                 -- code (kind='severity')
    resolution       TEXT,                                 -- code (kind='resolution') — финальное
    internal_note    TEXT,
  ...
  await db.pool.query(`CREATE INDEX IF NOT EXISTS idx_complaints_created ON tgbot.complaints (created_at)`);
  await db.pool.query(`CREATE INDEX IF NOT EXISTS idx_complaints_status  ON tgbot.complaints (status)`);
  await db.pool.query(`CREATE INDEX IF NOT EXISTS idx_complaints_agent   ON tgbot.complaints (agent_sd_id)`);
  await db.pool.query(`CREATE INDEX IF NOT EXISTS idx_complaints_product ON tgbot.complaints (product_name)`);

  // Медиа претензии. Байты лежат в public.files (Hub отдаёт через /file/:id); здесь — ссылка.
  await db.pool.query(`CREATE TABLE IF NOT EXISTS tgbot.complaint_files (
    id           BIGSERIAL PRIMARY KEY,
    ...
    file_ref     INT,                                      -- id в public.files
    tg_file_id   TEXT,                                     -- запасной идентификатор Telegram
    created_at   TIMESTAMPTZ NOT NULL DEFAULT now()
  )`);
  await db.pool.query(`CREATE INDEX IF NOT EXISTS idx_complaint_files_cid ON tgbot.complaint_files (complaint_id)`);

  // Сидирование справочника. Идемпотентно: правим label/звено/порядок без дублей.
  // Звенья (link): на какое звено цепочки указывает тип жалобы.
  const LINKS = [
    ['field',       'Поле · сырьё',            1],
  ...
  // Типы жалоб (19 из вашего справочника), каждый помечен звеном.
  const TYPES = [
  ...
  const seedDict = async (kind, code, label, sort, link) => {
    await db.pool.query(
  ...
  for (const [code, label, sort] of LINKS)        await seedDict('link', code, label, sort, null);
  for (const [code, label, link, sort] of TYPES)  await seedDict('type', code, label, sort, link);
  for (const [code, label, sort] of SEVERITIES)   await seedDict('severity', code, label, sort, null);
  for (const [code, label, sort] of RESOLUTIONS)  await seedDict('resolution', code, label, sort, null);
  for (const [code, label, sort] of CATEGORIES)   await seedDict('category', code, label, sort, null);
