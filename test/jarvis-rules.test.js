...
test('оживление сообщений: цифры подменять нельзя', () => {
  ...
  // Однозначные числа — это сроки и количества. Раньше они не проверялись,
  // и «ответь за 3 дня» могло стать «за 9 дней», пройдя защиту.
  const due = 'Карточка «Прайс»: срок прошёл 3 дня назад, ответа нет.';
  assert.ok(numbersKept(due, 'Слушай, «Прайс» висит уже 3 дня — глянешь?'));
  assert.ok(!numbersKept(due, 'Слушай, «Прайс» висит уже 9 дней — глянешь?'));
});
